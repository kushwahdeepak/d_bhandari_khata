...
function onSelectCustomer()
{
    ...
    $("#loan_super_admin_table").DataTable().ajax.reload(null, false);
}


$(function () {
    $("#loan_super_admin_table").DataTable({
        "pageLength": 10,
        lengthMenu: [
        [ 10, 25, 50, 100, 250, 500 ],
        [ '10', '25', '50', '100', '250', '500']
        ...
        "serverSide":true,  
        "ajax":{  
            url: site_url+"/loan/fetch_super_admin_details",   
            "data": function ( d ) 
                {
...
function updateGoldValue(item_id="") 
{
    ...
    $("#gold_value"+item_id).val(Math.round(orignal_gold));
}
...
function updateSilverValue(item_id="") 
{
    ...
    $("#silver_value"+item_id).val(Math.round(orignal_silver));
}


function readURLLoanSilver(input, input1=1, i=0)
{
...
function readURLLoanGold(input, input1=1, i=0)
{
    if (input.files && input.files[0]) 
    {
        var reader = new FileReader();
